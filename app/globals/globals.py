# globals/globals.py
# CONSTANTS
APP_VERSION = "0.0.4"
NO_DATA_DESCRIPTION = "None/Empty"
KNOWN_BUGS = [
    "App: If you click a button, the page jumps down to the very bottom",
    "App: The progress bar fills to quickly and doesn't really represent progress",
    "App: The file upload may not work on mobile Chrome (or Chrome based browsers like Brave). Workaround: use the 'view desktop page' option of your mobile browser",
    ]