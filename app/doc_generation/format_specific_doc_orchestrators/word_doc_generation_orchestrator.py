...
from doc_generation.type_group_generators.word_doc_chat_generator import generate_chat_word_docs
...
def generate_word_docs(all_data: AllData, user_options: UserOptions, on_progress=None) -> list[FileBuffer]:
    ...
    if has_chat:
        doc_buffers.extend(generate_chat_word_docs(all_data, user_options, on_progress))
    if has_non_chat:
        doc_buffers.extend(generate_nonchat_word_docs(all_data, user_options, on_progress))
